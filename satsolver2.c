	...
	for (int i = 0; i != last_edit; i++) {
		...
		switch (clen) {
			...
			case 1:
				...
				if (u_ispos) pconf[u_var / sbitstore] |= bit(u_var % sbitstore);
				else         nconf[u_var / sbitstore] |= bit(u_var % sbitstore);
				lit_propagate(cconf, u_lit);
	...
	for (int i = 1; i != last_edit; i++) {
		...
		switch (var_state(cconf, i)) {
			case 0b01:
				...
				pconf[i / sbitstore] |= bit(i % sbitstore);
				lit_propagate(cconf, i);
				last_edit = (i == 1) ? (n_vars + 1) : i;
				break;
			case 0b10:
				assert(i / sbitstore < ol_conf_size);
				assert(!e_occurrence_unsat(cconf, i) && e_occurrence_unsat(cconf, -i));
				nconf[i / sbitstore] |= bit(i % sbitstore);
				lit_propagate(cconf, -i);
				last_edit = (i == 1) ? (n_vars + 1) : i;
				break;
			case 0b00:
