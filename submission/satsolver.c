...
{
	int ** ols = ((int **) malloc((2 * formula->n_vars + 1) * sizeof *ols)) + formula->n_vars;
...
void print_formula(Formula * formula, FILE * stream)
{
	for (int var = 1; var <= formula->n_vars; var++) {
...
int main(int argc, char const *argv[])
{
	...
	if (formula == NULL) {
		...
		puts("Satisfiable!");
		print_formula(formula, stdout);
		if (fw != NULL) {
			print_formula(formula, fw);
		}
	}
